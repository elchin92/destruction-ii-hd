...
IDirectSoundBuffer *DSLoadSoundBuffer(IDirectSound *pDS, LPCTSTR lpName)
{
    ...
    if (DSGetWaveResource(NULL, lpName, &dsBD.lpwfxFormat, &pbWaveData, &dsBD.dwBufferBytes))
    {
        ...
        dsBD.dwFlags = DSBCAPS_LOCSOFTWARE | DSBCAPS_STATIC  | DSBCAPS_CTRLPAN | DSBCAPS_CTRLVOLUME | DSBCAPS_GETCURRENTPOSITION2;
...
BOOL SndObjIsPlaying(SNDOBJ *pSO)
{
    IDirectSoundBuffer *pDSB;
...
BOOL SndObjFirstPlayedLongest(SNDOBJ *pSO1, SNDOBJ *pSO2) {

	BOOL res = rand()%2==1;
    IDirectSoundBuffer *pDSB1,*pDSB2;
	
    if (pSO1 == NULL||pSO2 == NULL)
		return res;
	
    if (pDSB1 = pSO1->Buffers[pSO1->iCurrent])
		if (pDSB2 = pSO2->Buffers[pSO2->iCurrent])
		{
			HRESULT hres;
			unsigned long lpdwCapturePosition1, lpdwCapturePosition2; 
			unsigned long lpdwReadPosition1, lpdwReadPosition2  ;
			
			
			hres = IDirectSoundBuffer_GetCurrentPosition(pDSB1, &lpdwCapturePosition1, &lpdwReadPosition1);
			
			if (FAILED(hres))
				return res;

			hres = IDirectSoundBuffer_GetCurrentPosition(pDSB2, &lpdwCapturePosition2, &lpdwReadPosition2);			

			if (FAILED(hres))
				return res;
			if (lpdwReadPosition1>lpdwReadPosition2)
				return TRUE;
...
BOOL SetSPan(SNDOBJ *pSO,long pan){

	return SUCCEEDED(IDirectSoundBuffer_SetPan(pSO->Buffers[pSO->iCurrent],pan));

}
BOOL SetSVolume(HSNDOBJ pSO,long Volume){
	if(Volume>0)
		return FALSE;

	return SUCCEEDED(IDirectSoundBuffer_SetVolume(pSO->Buffers[pSO->iCurrent],Volume));

}
BOOL ResetPan(SNDOBJ *pSO){

	return SUCCEEDED(IDirectSoundBuffer_SetPan(pSO->Buffers[pSO->iCurrent],DSBPAN_CENTER));
